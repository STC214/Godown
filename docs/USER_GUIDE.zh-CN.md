...
> 适用于 `0.1.21-stage28` Windows x64 便携版。解压 ZIP 后直接运行 `gd3win.exe`，并始终让 `gd3-bt-runtime.exe` 与主程序位于同一目录。

## 1. 界面概览

主窗口由来源输入区、任务工具栏、任务表格、任务详情和状态栏组成。

程序默认使用暗色主题。任务表格上方的“名称、状态、进度、大小、速度、目录”表头可点击排序；当前列使用 `▲` 或 `▼` 标记方向。主窗口、文件、任务栏和托盘使用同一内置应用图标，启动时不会额外打开命令行窗口。

| 操作 | 作用 |
| --- | --- |
| 添加地址 | 识别输入内容并创建 HTTP、M3U8 或 Magnet 任务 |
...
支持 `ftp://` 普通 FTP、`ftps://` 隐式 TLS 和 `ftpes://` 显式 TLS 的单文件及目录地址。目录可使用 `ftp://host/path/dir/`，未带末尾斜杠时程序也会在文件探测失败后尝试识别目录。地址可包含用户名与密码，例如 `ftp://user:password@host/path/file.zip`；任务列表和持久任务 URL 会移除凭据，任务数据库中的 FTP 密码使用 Windows 当前用户 DPAPI 加密。旧版本留下的明文凭据会在首次加载时自动迁移。未填写账户时使用匿名登录。

FTP 下载使用 `.part` 文件断点续传；服务器不接受 REST 恢复时会自动从头重试。服务器不支持 SIZE 命令时仍可下载单文件和目录，无法确定长度的文件会下载至服务器正常结束传输。目录任务创建时会递归保存最多 100,000 个目录项，下载时保留层级和空目录、跳过符号链接，并仅在任务完成标记与本地文件 SHA-256 同时匹配时跳过已完成文件。未完成文件会逐个续传；未知大小的旧 `.part` 会从头下载。输出名称会按 Windows 规则清理非法字符、设备名和尾随点/空格；清理后发生重名会停止创建任务。旧版本创建的无所有权标记目录会原样保留，任务自动改用独立的新输出目录并暂停，手动开始后重新下载；旧目录不会被清理。全局限速和重试次数同样生效。代理设置为空时直连，设置代理时仅支持 SOCKS5/SOCKS5H。
...
解析 torrent 后会显示文件路径、大小、选择状态和优先级。

- **全选**：选择全部可下载文件。
- **清空**：清空选择。
- **反选**：反转当前选择。
- **低 / 普通 / 高**：为当前选中的列表行设置实际下载优先级；新选中的文件默认为普通优先级。
- 底部汇总显示所选文件数与总大小。

至少需要选择一个文件。Padding 文件不会出现在普通选择列表中；共享边界块所需的 padding 或未选文件字节会写入任务自有的隐藏目录，不计入所选文件进度。

任务开始后仍可选中 BT 任务并选择 **BT 文件**。程序会暂停对应 BT runtime、保存最新校验进度、串行应用新的文件选择和优先级，然后自动恢复此前处于活动状态的任务；原本手动暂停的任务保持暂停。
...
| --- | --- |
| 保存目录 | 新任务的默认保存目录 |
| 代理地址 | 支持 HTTP(S) 与 SOCKS5 代理地址 |
| 请求标头 | 每行一个请求头，供支持的 HTTP 请求使用 |
| Cookie | HTTP/M3U8 请求使用的 Cookie 文本 |
| 分块数 | HTTP 分块数量 |
| 重试次数 | 失败重试次数 |
| 限速 | 通用下载限速；`0` 表示不限速 |
| 最大并发任务 | 普通下载任务并发槽位数 |
...
| Sequential download | 关闭 | 按文件优先级依次请求文件，当前文件之外不主动请求数据块 |
...
```powershell
Get-FileHash -Algorithm SHA256 .\GhostDownloader-0.1.21-stage28-windows-x64-portable.zip
```

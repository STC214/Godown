...
func taskSnapshotPayload(scheduler *core.Scheduler) map[string]any {
	...
	for _, task := range snapshots {
		...
		tasks = append(tasks, map[string]any{
			...
			"canPause":      task.Status == core.StatusRunning || task.Status == core.StatusSeeding || task.Status == core.StatusWaiting,
			"canOpenFile":   (task.Status == core.StatusCompleted || task.Status == core.StatusSeeding) && fileErr == nil,

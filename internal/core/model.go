...
func (t Task) CleanupFiles() error {
	...
	if t.PackID == "bt" {
		...
		if err := removeTaskOwnedPath(t.Path, outputFile+".part"); err != nil {
			return err
		}
		if err := removeTaskOwnedPath(t.Path, filepath.Join(t.Path, ".gd3_bt", t.ID)); err != nil {
			return err
		}
		if err := removeIfExists(outputFile + ".torrent"); err != nil {
			return err
		}
		return nil
	}
...
// TaskResetter is an optional Worker capability used by Redownload. It lets a
// pack clear its own durable runtime checkpoint while retaining immutable task
// metadata such as a BitTorrent file selection.
type TaskResetter interface {
	ResetTask(task Task) (Task, error)
}
...
func cloneTask(task Task) Task {
	task.Stage.Headers = cloneStringMap(task.Stage.Headers)
	task.Stage.State = cloneStringMap(task.Stage.State)
	return task
}

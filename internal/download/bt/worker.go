...
func newClientWithPortFallback(config *torrent.ClientConfig) (*torrent.Client, error) {
	...
	if err == nil || !listenBindFailure(err) {
		return client, err
	}
	...
	for attempt := 0; attempt < 4; attempt++ {
		...
		if fallbackErr == nil {
			if requestedPort != 0 {
				slog.Warn("BitTorrent listen port unavailable; selected an ephemeral port", "requestedPort", requestedPort)
			}
			return client, nil
		}
		lastErr = fallbackErr
		if !listenBindFailure(fallbackErr) {
			break
		}
	}
	return nil, fmt.Errorf("bind BitTorrent port %d: %w (ephemeral retries: %v)", requestedPort, err, lastErr)
}

func listenBindFailure(err error) bool {
	if errors.Is(err, syscall.EADDRINUSE) || errors.Is(err, syscall.EACCES) {
		return true
	}
	var errno syscall.Errno
	return errors.As(err, &errno) && (errno == syscall.Errno(10048) || errno == syscall.Errno(10013))
}
